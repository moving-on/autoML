...
class NetworkManager:
    ...
    def get_rewards(self, model_constructor, actions):
        '''
        Creates a subnetwork given the actions predicted by the controller RNN,
        trains it on the provided dataset, and then returns a reward.

        Args:
            model_constructor: An instance of ModelConstrctor. It is responsible
                for creating the model given a configutation 
        ...
        with tf.Session(graph=tf.Graph()) as network_sess:
            ...
            model = model_constructor.build_model(actions)  # type: Model
            model.summary()
