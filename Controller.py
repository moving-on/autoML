...
class Controller:
    ...
    def __init__(self, policy_session, num_layers, state_space,
        ...
        self.state_size = self.state_space.num_spaces
    ...
    def get_action(self, state):
        ...
        if np.random.random() < self.exploration:
            ...
            for i in range(self.state_size * self.num_layers):
                ...
                sample = state_.index_map[sample[0]]
            ...
            size = self.state_space.space_dim
    ...
    def build_policy_network(self):
        with self.policy_session.as_default():
            ...
            def dense_layer(input, units, state_id, name):
                with tf.variable_scope("dense_layer_%d" %state_id, reuse=tf.AUTO_REUSE):
                    classifier = tf.layers.dense(input, units=units, name=name)
                return classifier

            def embedding_layer(input):
                with tf.variable_scope("embedding", reuse=tf.AUTO_REUSE):
                    weights = tf.get_variable("state_embedding", shape=[self.state_space.space_dim, self.embedding_dim],
                                                      initializer=tf.initializers.random_uniform(-1., 1.))
                    embedding = tf.nn.embedding_lookup(weights, input)
                return embedding
                ...
                with tf.variable_scope('policy_network'):
                    ...
                    for i in range(self.state_size * self.num_layers):
                        ...
                        with tf.name_scope('controller_output_%d' % i):
                            ...
                            classifier = dense_layer(outputs[:, -1, :], units=size, state_id=state_id, name='classifier_%d' % (i))
                            ...
                            cell_input = tf.add(cell_input, space_dim)  # we avoid using 0 so as to have a "default" embedding at 0th index
                            space_dim += size
                            space_dim = space_dim % self.state_space.space_dim

                            # embedding lookup of this state using its state weights ; reuse weights
                            cell_input = embedding_layer(cell_input)
            ...
            with tf.name_scope('losses'):
                ...
                self.all_input = tf.placeholder(tf.int32, shape=(None, self.state_size * self.num_layers), name="all_input")
                all_emb = embedding_layer(self.all_input)
                    #per_emb = tf.transpose(per_emb, (0, 2, 1))

                outputs, final_state = tf.nn.dynamic_rnn(nas_cell,
                                                         all_emb,
                                                         initial_state = nas_cell.zero_state(batch_size=1, dtype=tf.float32),
                                                         dtype=tf.float32)
                cross_entropy_loss = 0
                for i in range(self.state_size * self.num_layers):
                    ispace = self.state_space[i]
                    size = ispace.size
                    state_id = i % self.state_size
                    classifier = dense_layer(outputs[:, i, :], units=size, state_id=state_id, name='classifier_%d' % (i))
                    with tf.name_scope('state_%d' % (i + 1)):
                        labels = tf.placeholder(dtype=tf.float32, shape=(None, size), name='cell_label_%d' % i)
                        self.policy_labels.append(labels)
                        per_loss = tf.nn.softmax_cross_entropy_with_logits_v2(logits=classifier, labels=labels)
                        tf.summary.scalar('state_%d_per_loss' % (i + 1), tf.reduce_mean(per_loss))
                    cross_entropy_loss += per_loss
    ...
    def train_step(self, first_state):
        '''
        ...
        state_list = self.state_space.parse_state_space_list(states, local=True)
        print state_list
        first_state_list = self.state_space.parse_state_space_list([first_state], local=True)
        print first_state_list
        encode_state = self.state_space.encode_state_space_list(states)
        print encode_state
        encode_first_state = self.state_space.encode_state_space_list([first_state])
        print encode_first_state
        encode_all_input = [encode_first_state + encode_state[:-1]]
        print encode_all_input
        ...
        feed_dict = {
            ...
            self.all_input: encode_all_input
        }
